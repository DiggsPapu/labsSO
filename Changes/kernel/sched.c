...
need_resched_nonpreemptible:
	...
	if (prev->policy == SCHED_CASIO || next->policy == SCHED_CASIO){ 
		if (prev->policy == SCHED_CASIO && next->policy == SCHED_CASIO){ 
			//logs1 
			snprintf(msg, CASIO_MSG_SIZE, "CONFIG_SCHED_POLICY LOG1 prev->(%d:%d), next->(%d:%d)",  prev->casio_id, prev->pid, next->casio_id, next->pid);
			register_casio_event(sched_clock(), msg, CASIO_MSG) ; 
		} else { 
			if (prev->policy == SCHED_CASIO){ 
				//logs2 
				snprintf(msg, CASIO_MSG_SIZE, "CONFIG_SCHED_POLICY LOG2 prev->(%d:%d), next->(-1:%d)",  prev->casio_id, prev->pid, next->pid);
				register_casio_event(sched_clock(), msg, CASIO_MSG) ; 
			} else { 
				//logs3
				snprintf(msg, CASIO_MSG_SIZE, "CONFIG_SCHED_POLICY LOG2 prev->(-1:%d), next->(%d:%d)",  prev->pid, next->casio_id, next->pid);
				register_casio_event(sched_clock(), msg, CASIO_MSG) ;  
			} 
		} 
